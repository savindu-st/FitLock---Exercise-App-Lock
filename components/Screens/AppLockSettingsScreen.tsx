...
const AppLockSettingsScreen: React.FC<AppLockSettingsScreenProps> = ({ apps, onUpdateApp, onRequestCamera }) => {
      ...
      <div className={`p-4 rounded-xl border mb-4 ${cameraGranted === true
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${cameraGranted ? 'bg-green-100' : 'bg-amber-100'
              }`}>
              {cameraGranted ? (
            ...
            <div>
              ...
              <p className={`text-xs ${cameraGranted ? 'text-green-600' : 'text-amber-600'}`}>
                {cameraGranted === null ? 'Checking...' : cameraGranted ? 'Granted' : 'Not granted'}
              </p>
            </div>
          </div>
          {!cameraGranted && (
